...
inline function setValidLicense(isValid)
{
    ...
    if(isValid)
    {
        ...
        GlobalMute.setAttribute(0, 0);
    }
    else
    {
        //not a valid license...
        Console.print("not valid doing something");
        AuthorisationDialogue.set("visible", true);
        AuthState = false;
        AuthoriseInInst.setValue(0);
...
inline function checkOnLoad()
{
    ...
    if(pData)    
    {
        ...
        Console.print("not a vlaid license..");
...
//  in demo mode we allow it to make noises........
...
AuthorisationDialogue.setTimerCallback(function()
{
    Console.print(Engine.getUptime() - engineStartTime );
      ...
      if (AuthState == false)
      {
          //Ok we waited 15 mins and its not authorised. shut it down..
          GlobalMute.setAttribute(0, 1);
          AuthorisationDialogue.stopTimer();
          //and show the authorisation dialog
          // but this time no option to run in demo mode..
...
inline function onRandomiseControl(component, value)
{
    if (value)
    {
        ...
        for (idx = 0; idx < 3; idx++)
        {
            ...
            VolumeKnobs[idx].setValue((Math.random()*26)-20);
            VolumeKnobs[idx].changed();
            
            PitchKnobs[idx].setValue((Math.random()*8)-4);
            PitchKnobs[idx].changed();
            
            FilterOnOffs[idx].setValue(Math.round(Math.random()));
            FilterOnOffs[idx].changed();
            
            FreqKnobs[idx].setValue(Math.round(Math.random()*1400)+25);
            FreqKnobs[idx].changed();
            
            ShapeOnOffs[idx].setValue(Math.round(Math.random()));
            ShapeOnOffs[idx].changed();
            
            ShapeDrives[idx].setValue((Math.random()*26)+14);
            ShapeDrives[idx].changed();
            
            ReverbOnOffs[idx].setValue(Math.round(Math.random()));
            ReverbOnOffs[idx].changed();
            
            if(Math.random() > 0.7)
            {
                DelayOnOffs[idx].setValue(Math.round(Math.random()));
            }else{
                DelayOnOffs[idx].setValue(0);
            };
            DelayOnOffs[idx].changed();
            
            CompOnOffs[idx].setValue(Math.round(Math.random()));
            CompOnOffs[idx].changed();
        };
...
inline function onAuthoriseInInstControl(component, value)
{
    ...
    if (AuthState == false){
        AuthorisationDialogue.set("visible", 0);
        GlobalMute.setAttribute(0, 0);
    };
};
